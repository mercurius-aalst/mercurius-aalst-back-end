...
internal sealed class TournamentRegistrationRosterMemberConfiguration : IEntityTypeConfiguration<TournamentRegistrationRosterMember>
{
    ...
    {
        entity.ToTable("roster_members", "tournament");
        ...
        entity.HasOne(member => member.Tournament)
            .WithMany()
            .HasForeignKey(member => member.TournamentId)
            .OnDelete(DeleteBehavior.Cascade);
        entity.HasIndex(member => new { member.TournamentId, member.UserId })
            .IsUnique()
            .HasDatabaseName("IX_TournamentRosterMembers_TournamentId_UserId_PendingActive");
        entity.HasIndex(member => new { member.TournamentId, member.TeamId, member.UserId });
