...
public interface ITeamEventPublisher
{
    ...
    Task RosterSelectionChangedAsync(Guid teamId, Guid rosterMemberId, Guid affectedUserId, string status);
...
public class SignalRTeamEventPublisher : ITeamEventPublisher
{
    ...
    public Task RosterSelectionChangedAsync(Guid teamId, Guid rosterMemberId, Guid affectedUserId, string status)
    {
        return _hubContext.Clients.Groups([GetUserGroup(affectedUserId), GetTeamGroup(teamId)])
            .SendAsync(
                "TournamentRosterSelectionChanged",
                new TournamentRosterSelectionChangedEvent(teamId, rosterMemberId, affectedUserId, status));
    }
...
public class NullTeamEventPublisher : ITeamEventPublisher
{
    ...
    public Task RosterSelectionChangedAsync(Guid teamId, Guid rosterMemberId, Guid affectedUserId, string status) => Task.CompletedTask;
...
public record TournamentRosterSelectionChangedEvent(Guid TeamId, Guid RosterMemberId, Guid UserId, string Status); 
