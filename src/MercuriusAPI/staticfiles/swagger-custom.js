...
(function () {
    ...
    const reloadSchemaOnAuth = (swagger) => {
        const getCurrentUrl = () => {
            try {
                ...
                var urlSpan = document.getElementsByClassName("url")[0];
                return urlSpan.textContent;//"http://localhost:5019/swagger/v1/swagger.json";//undefined;
            }
        }
        const reload = () => {
    ...
    window.addEventListener('load', () => setTimeout(overrider, 100), false);
}());
