...
public interface ITeamEventPublisher
{
    ...
    Task RosterConfirmationChangedAsync(Guid teamId, Guid notificationId, Guid affectedUserId, string status);
...
public class SignalRTeamEventPublisher : ITeamEventPublisher
{
    ...
    public Task RosterConfirmationChangedAsync(Guid teamId, Guid notificationId, Guid affectedUserId, string status)
    {
        return _hubContext.Clients.Groups([GetUserGroup(affectedUserId), GetTeamGroup(teamId)])
            .SendAsync("TournamentRosterConfirmationChanged", new TournamentRosterConfirmationChangedEvent(teamId, notificationId, affectedUserId, status));
    }
...
public class NullTeamEventPublisher : ITeamEventPublisher
{
    ...
    public Task RosterConfirmationChangedAsync(Guid teamId, Guid notificationId, Guid affectedUserId, string status) => Task.CompletedTask;
...
public record TournamentRosterConfirmationChangedEvent(Guid TeamId, Guid NotificationId, Guid UserId, string Status);
